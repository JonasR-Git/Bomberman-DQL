...
def setup_training(self):
    ...
    self.losses = []
    self.moves_last_x_games = deque(maxlen=500)
    self.current_moves_counter = 0
    self.coins_collected = 0
    self.coins_collected_round = deque(maxlen=300)
    self.invalid_moves = 0
    self.invalid_moves_last_x_games = deque(maxlen=500)
    self.bombs_placed = 0
    self.bombs_placed_last_x_games = deque(maxlen=500)
...
def end_of_round(self, last_game_state, last_action, events):
    ...
    if (self.episode_counter - 100) % 500 == 0:
...
def write_stats_to_file(self, filename="training_stats.txt"):
    with open(filename, "a") as f: 
        average_score = np.mean(self.game_score_arr[-500:])
        average_loss = np.mean(self.losses[-500:])
